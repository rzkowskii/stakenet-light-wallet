...
bool CheckSignatureEncoding(
    const std::vector<unsigned char>& vchSig, unsigned int flags, ScriptError* serror)
{
...
bool static CheckPubKeyEncoding(
    const valtype& vchPubKey, unsigned int flags, const SigVersion& sigversion, ScriptError* serror)
{
...
bool EvalScript(std::vector<std::vector<unsigned char>>& stack, const CScript& script,
    unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion,
    ScriptError* serror)
{
        ...
        while (pc < pend) {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
                ...
                case OP_HASH256: {
                    ...
                    valtype& vch = stacktop(-1);
                    valtype vchHash(
                        (opcode == OP_RIPEMD160 || opcode == OP_SHA1 || opcode == OP_HASH160) ? 20
                                                                                              : 32);
                ...
                case OP_CHECKMULTISIGVERIFY: {
                    ...
                    if (nOpCount > MAX_OPS_PER_SCRIPT)
                        return set_error(serror, SCRIPT_ERR_OP_COUNT);
                    int ikey = ++i;
                    // ikey2 is the position of last non-signature item in the stack. Top stack item
                    // = 1. With SCRIPT_VERIFY_NULLFAIL, this is used for cleanup if operation
                    // fails.
...
namespace {
    ...
    private:
        const CTransaction&
            txTo; //!< reference to the spending transaction (the one being serialized)
...
bool TransactionSignatureChecker::CheckSig(const std::vector<unsigned char>& vchSigIn,
    ...
    const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (witversion == 0) {
        if (program.size() == WITNESS_V0_SCRIPTHASH_SIZE) {
            // Version 0 segregated witness program: SHA256(CScript) inside the program, CScript +
            // inputs in witness
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey,
    const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker,
    ScriptError* serror)
{
    ...
    const CScriptWitness* witness, unsigned int flags)
{
    ...
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
        return WitnessSigOps(
            witnessversion, witnessprogram, witness ? *witness : witnessEmpty, flags);
    }

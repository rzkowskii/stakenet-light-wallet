...
bool FileLock::TryLock()
{
    ...
    if (!LockFileEx(
            hFile, LOCKFILE_EXCLUSIVE_LOCK | LOCKFILE_FAIL_IMMEDIATELY, 0, 0, 0, &overlapped)) {
        reason = GetErrorReason();
        return false;
    }
    return true;
...
bool LockDirectory(
    const boost::filesystem::path& directory, const std::string lockfile_name, bool probe_only)
{

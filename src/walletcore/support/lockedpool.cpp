...
void* Arena::alloc(size_t size)
{
    ...
    // Pick a large enough free-chunk. Returns an iterator pointing to the first element that is not
    // less than key. This allocation strategy is best-fit. According to "Dynamic Storage
    // Allocation: A Survey and Critical Review", Wilson et. al. 1995,
    // http://www.scs.stanford.edu/14wi-cs140/sched/readings/wilson.pdf, best-fit and first-fit
...
{
    std::cout << "0x" << std::hex << std::setw(16) << std::setfill('0') << base << " 0x" << std::hex
              << std::setw(16) << std::setfill('0') << sz << " 0x" << used << std::endl;
}
...
LockedPool::LockedPool(
    std::unique_ptr<LockedPageAllocator> allocator_in, LockingFailed_Callback lf_cb_in)
...
bool LockedPool::new_arena(size_t size, size_t align)
{
    ...
    if (locked) {
        ...
        if (!lf_cb()) { // If the callback returns false, free the memory and fail, otherwise
                        // consider the user warned and proceed.
...
LockedPool::LockedPageArena::LockedPageArena(
    LockedPageAllocator* allocator_in, void* base_in, size_t size_in, size_t align_in)

    ...
    if (fUpdateID) {
        ...
        if (!CMnemonic::Check(ssMnemonicTmp)) {
            throw std::runtime_error(std::string(__func__) + ": invalid mnemonic: `"
                + std::string(ssMnemonicTmp.c_str()) + "`");
        }
...
bool CHDChain::GetMnemonic(
    SecureVector& vchMnemonicRet, SecureVector& vchMnemonicPassphraseRet) const
{
...
bool CHDChain::GetMnemonic(SecureString& ssMnemonicRet, SecureString& ssMnemonicPassphraseRet) const
{
    ...
    ssMnemonicPassphraseRet
        = SecureString(vchMnemonicPassphrase.begin(), vchMnemonicPassphrase.end());
...
void CHDChain::SetChainScope(uint32_t nPurpose, const CHDChainScope &scope)
{
    LOCK(cs_scopes);
    mapChainScopes[nPurpose] = scope;
}

CHDChainScope CHDChain::GetChainScope(uint32_t nPurpose)
{
    LOCK(cs_scopes);
    auto scope = mapChainScopes[nPurpose];
    scope.nPurpose = nPurpose;
    return scope;
}

void CHDChain::DeriveChildExtKey(uint32_t nPurpose, uint32_t nCoinType, uint32_t nAccountIndex,
...
void CHDChainScope::AddAccount(uint32_t coinType)
{
    LOCK(cs_accounts);
...
bool CHDChainScope::GetAccount(uint32_t coinType, uint32_t nAccountIndex, CHDAccount& hdAccountRet) const
{
    LOCK(cs_accounts);
    if (accountsByCoin.count(coinType) > 0) {
        const auto& accounts = accountsByCoin.at(coinType);
...
bool CHDChainScope::SetAccount(uint32_t coinType, uint32_t nAccountIndex, const CHDAccount& hdAccount)
{
    LOCK(cs_accounts);
    if (accountsByCoin.count(coinType) > 0) {
        auto& accounts = accountsByCoin[coinType];
...
size_t CHDChainScope::CountAccounts(uint32_t coinType)
{
    LOCK(cs_accounts);
...
{
    return strprintf(
                "m/%d'/%d'/%d'/%d/%d", nPurpose, nCoinType, nAccountIndex, nChangeIndex, extPubKey.nChild);
}

void CHDChainScope::SetNull()
{
